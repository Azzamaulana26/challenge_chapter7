...
module.exports = {
  ...
  // Fungsi untuk menampilkan halaman aktivasi akun
  getActivate: (req, res) => {
     let { token } = req.query;
  ...
  // Fungsi untuk melakukan aktivasi akun
  activate: (req, res) => {
    let { token } = req.query;
    jwt.verify(token, JWT_SECRET_KEY, async (err, decoded) => {
      if (err) {
  ...
  // Fungsi untuk memberikan informasi tentang pengguna yang sedang masuk
  whoami: (req, res, next) => {
    res.status(200).json({
      status: true,
      message: 'OK',
